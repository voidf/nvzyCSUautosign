...
@logger.catch
async def agetold(ses: aiohttp.ClientSession):
    await aauth_session(ses)
    lnk = 'https://wxxy.csu.edu.cn/ncov/wap/default/index?from=history'
    ...
    async with aiohttp.ClientSession() as ses:
        while True:
            try:
                ...
                sign_res = await asign_in(ses, await agetold(ses))
                if sign_res[0] == "{":
                    print(sign_res)
...
def sign_in(je):
    ...
    try:
        ...
        return ""
@logger.catch
async def asign_in(ses: aiohttp.ClientSession, je):
    await aauth_session(ses)
    url = "https://wxxy.csu.edu.cn/ncov/wap/default/save"
    je["id"] = int(je["id"])
    je["date"] = str_time("%Y%m%d")
...
@logger.catch
async def aauth_session(ses: aiohttp.ClientSession):
    await ar2(ses, auth['userName'], auth['passWord'])
